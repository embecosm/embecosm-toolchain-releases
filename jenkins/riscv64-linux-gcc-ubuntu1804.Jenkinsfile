...
String PKGVERS = "riscv64-embecosm-linux-gcc-ubuntu1804-${CURRENTTIME}"
...
node('builder') {
  ...
  stage('Checkout') {
    ...
    // QEMU 8 does not build on Ubuntu 18.04 due to an older version of Python, so fall back to QEMU 7.2
    dir('qemu') {
      checkout([$class: 'GitSCM',
          branches: [[name: '*/stable-7.2']],
          extensions: [[$class: 'CloneOption', shallow: true]],
