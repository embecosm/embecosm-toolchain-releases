...
node('macbuilder') {
  ...
  stage('Build') {
    sh script: "BUGURL='${BUGURL}' PKGVERS='${PKGVERS}' arch -x86_64 ./stages/build-riscv32-clang.sh"
  }
  ...
  stage('Test') {
    dir('build/binutils-gdb') {
      sh script: 'arch -x86_64 make check-gas', returnStatus: true
      sh script: 'arch -x86_64 make check-ld', returnStatus: true
      sh script: 'arch -x86_64 make check-binutils', returnStatus: true
    ...
    sh script: '''arch -x86_64 ./stages/test-llvm.sh'''
    dir('build/llvm') {
